...
import { ishttps } from '../../static/https'
console.log(ishttps, 'dddasa')
// 环境的切换
if (process.env.NODE_ENV == 'development') {
    axios.defaults.baseURL = '//res.ttz.com/';
    ...
    response => {
        if (response.data.Code === 200 || response.data.code === 'success') {
            ...
            if (response.data.Code === 9999) {
                ...
                removeToken('ttz_videoCo')
                store.commit('user/SET_TOKEN', '')
                store.commit('user/SET_INFO', '')
                store.commit('video/setVideoToken', '')
                ...
                tabCode()
                return Promise.reject(response);
            } else if (response.data.Code === 672) {   //////游戏试玩验证码
                return Promise.resolve(response)
                ...
                if (noneList.indexOf(window.location.pathname) == -1) {
                    store.commit('app/getFrame', true)
                    store.commit('app/getFrameMsg', response.data.Msg)
                    store.commit('app/getFrameIcon')
                    store.commit('app/getFrameState', 0)
